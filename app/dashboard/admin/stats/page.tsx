...
export default async function StatsPage() {
  ...
  const stats = [
    { label: t.events, value: totalEvents || 0, emoji: '📅', color: 'bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400' },
    { label: t.members, value: totalUsers || 0, emoji: '👥', color: 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400' },
    { label: t.registrations, value: totalRegistrations || 0, emoji: '✅', color: 'bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-400' },
    { label: t.average, value: totalEvents ? Math.round((totalRegistrations || 0) / totalEvents) + '/event' : '0', emoji: '📊', color: 'bg-orange-50 dark:bg-orange-900/20 text-orange-700 dark:text-orange-400' },
  ]
    ...
    <div className="p-4 md:p-6 dark:bg-gray-950 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="card p-4">
          <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">{t.popular}</h2>
          <div className="space-y-3">
            {eventsByPopularity.length === 0 && <p className="text-sm text-gray-400 dark:text-gray-500">{t.none}</p>}
              ...
              return (
                <Link key={e.id} href={`/dashboard/events/${e.id}`} className="block">
                  <div className="flex items-center gap-3">
                    <span className="text-sm font-bold text-gray-300 dark:text-gray-600 w-4">{i + 1}</span>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium truncate dark:text-white">{e.title}</p>
                      <div className="flex items-center gap-2 mt-1">
                        <div className="flex-1 bg-gray-100 dark:bg-gray-800 rounded-full h-1.5">
                          <div className="bg-brand-600 h-1.5 rounded-full" style={{ width: `${pct}%` }} />
                        </div>
                        <span className="text-xs text-gray-500 dark:text-gray-400 shrink-0">{count}/{e.max_attendees}</span>
                      </div>
                    </div>
                  </div>
                </Link>
        ...
        <div className="card p-4">
          <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">{t.upcoming}</h2>
          <div className="space-y-3">
            {upcoming.length === 0 && <p className="text-sm text-gray-400 dark:text-gray-500">{t.noneUpcoming}</p>}
            {upcoming.map(e => {
              const count = (e.registrations as any)?.[0]?.count || 0
              return (
                <Link key={e.id} href={`/dashboard/events/${e.id}`} className="flex items-center gap-3 group">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate dark:text-white group-hover:text-brand-600 dark:group-hover:text-brand-400">{e.title}</p>
                    <p className="text-xs text-gray-400 dark:text-gray-500">
                      {format(new Date(`${e.date}T${e.time}`), "d MMM 'à' HH'h'mm", { locale: dateLocale })} · {count} {t.registrations.toLowerCase()}
                    </p>
                  </div>
                  <span className="text-xs text-gray-400 dark:text-gray-500 shrink-0">{(categoryLabels as any)[e.category]}</span>
                </Link>
              )
            })}
          </div>
